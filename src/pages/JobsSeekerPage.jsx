...
const PROMO_IMG = "https://res.cloudinary.com/danwfu3n6/image/upload/v1763809031/luca-maffeis-iY_cqJome-A-unsplash_wrbehp.jpg";

/* new local images used for the requested sections */
const OVERSEAS_COLLAGE = "/mnt/data/Screenshot 2025-11-22 at 5.09.44 PM.png";
const COUNTRIES_BG = "https://res.cloudinary.com/danwfu3n6/image/upload/v1763813161/red-john-hosJZJN7HLY-unsplash_p9b277.jpg";

/* ---------------- Demo data (replace with real APIs) ---------------- */
const SAMPLE_JOBS = [
  { id: "J-001", title: "Excavator Operator", sector: "Mining", company: "OreWorks", location: "Jharsuguda", salary: "₹28k–₹35k", exp: "2–5 yrs", tags: ["Night", "On-site"] },
  { id: "J-002", title: "Furnace Operator", sector: "Steel & Aluminium", company: "Metallix", location: "Jamshedpur", salary: "₹30k–₹40k", exp: "1–4 yrs", tags: ["Rotational"] },
  { id: "J-003", title: "Turbine Technician", sector: "Power & Green Energy", company: "GreenGrid", location: "Bengaluru", salary: "₹35k–₹50k", exp: "3–6 yrs", tags: ["Certified"] },
];
...
const TRAININGS = [
  { id: "T1", title: "Excavator Simulator", duration: "2 days", type: "Practical" },
  { id: "T2", title: "Site Safety & Induction", duration: "1 day", type: "Cert" },
];
...
export default function JobSeekersPage() {
  ...
  const jobs = useMemo(() => {
    ...
    return SAMPLE_JOBS.filter((j) => {
      if (sector !== "all" && j.sector.toLowerCase() !== sector.toLowerCase()) return false;
      if (location && !j.location.toLowerCase().includes(location.toLowerCase())) return false;
      if (!q) return true;
      return [j.title, j.company, j.location].some((f) => f.toLowerCase().includes(q));
    });
  ...
  return (
    <main className="min-h-screen text-white antialiased bg-linear-to-b from-slate-900 via-black to-black">
      {/* HERO */}
      <header aria-label="Hero" className="relative overflow-hidden" style={{ minHeight: "90vh" }}>
        <video
        ...
        <div className="relative z-10 max-w-7xl mt-44 mx-auto px-6 py-20">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight leading-tight">Get placed faster. Work safer.</h1>
            <p className="mt-4 text-lg text-gray-200">
              Blue Global Force connects trained blue-collar talent with responsible employers across Mining, Steel, Power, Shipping, EV and Construction.
            </p>
          </div>
          ...
          >
            ...
            >
              ...
              {memoSectors.map((s) => <option key={s.id} value={s.title}>{s.title}</option>)}
            </select>
            <input
              value={location}
            ...
            />
            <button className="ml-2 rounded-full bg-linear-to-r from-teal-400 to-violet-500 text-black px-5 py-2 font-semibold">Search</button>
          </form>
          ...
          <div className="mt-4 text-sm text-gray-300 flex gap-4 items-center">
            <span className="inline-flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full">
              <span className="w-2 h-2 rounded-full bg-teal-300" /> Urgent hires: 300+
            </span>
            <span className="text-gray-400">Need help? <a href="#contact" className="text-teal-300 hover:underline">Contact us</a></span>
          </div>
        </div>
      </header>
      ...
      <div className="px-6 py-12 lg:py-20 space-y-12">
        ...
        <section className="max-w-7xl mx-auto bg-white/5 border border-white/6 rounded-2xl p-6 lg:flex lg:items-center lg:gap-6">
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1">Narrow by sector, location or experience to surface the best matches.</p>
          </div>

          <div className="mt-4 lg:mt-0 flex-1 flex gap-3 items-center">
            <select className="bg-black/10 px-3 py-2 rounded-md text-sm" value={sector} onChange={(e) => setSector(e.target.value)}>
              <option value="all">All sectors</option>
              {memoSectors.map((s) => <option key={s.id} value={s.title}>{s.title}</option>)}
            </select>

            <input className="flex-1 px-3 py-2 rounded-md bg-black/10 text-sm" placeholder="City / State" value={location} onChange={(e) => setLocation(e.target.value)} />
            ...
            <button onClick={() => { setSector("all"); setLocation(""); setQuery(""); }} className="px-4 py-2 rounded-md border border-white/10 text-sm">Reset</button>
          </div>

          <div className="mt-3 lg:mt-0 lg:ml-auto text-sm text-gray-300">Matches: <span className="text-white font-medium">{jobs.length}</span></div>
        </section>
        ...
        <section className="max-w-7xl mx-auto bg-linear-to-r from-slate-900 to-slate-800 border border-white/6 rounded-2xl p-6">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Highlighted and urgent openings</p>
            </div>

            <div className="hidden md:flex gap-3">
              <Link to="/jobs" className="px-4 py-2 rounded-full bg-white/6">Browse all jobs</Link>
              <Link to="/trainings" className="px-4 py-2 rounded-full bg-white/6">Training</Link>
            </div>
          </div>

          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobs.length ? jobs.map((j) => (
              <article key={j.id} className="rounded-xl p-4 bg-black/10 border border-white/6 hover:scale-[1.01] transition">
                <div className="flex items-start gap-4">
                  <div className="w-14 h-14 rounded-lg flex items-center justify-center bg-linear-to-br from-sky-400 to-indigo-600 text-white font-bold">
                    {j.title.split(" ").slice(0,2).map(w=>w[0]).join("")}
                  </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {j.tags.map((t) => <span key={t} className="text-xs bg-white/6 px-2 py-1 rounded-full">{t}</span>)}
                </div>

                <div className="mt-4 flex gap-2">
                  <button className="flex-1 rounded-full px-4 py-2 bg-linear-to-r from-teal-400 to-violet-500 text-black font-semibold">Quick Apply</button>
                  <button className="px-3 py-2 rounded-md border border-white/10">Details</button>
                </div>
              </article>
            )) : (
              <div className="text-gray-400 p-6">No roles found. Try removing filters or broaden your search.</div>
            )}
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto bg-white/4 border border-white/6 rounded-2xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-300 mt-1">A simple three step process from search to deployment.</p>
          </div>
          ...
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-lg bg-black/6 border border-white/6">
              ...
              <p className="text-sm text-gray-300 mt-2">Filter by role, sector & location. Save favourites.</p>
            </div>

            <div className="p-4 rounded-lg bg-black/6 border border-white/6">
              <div className="text-teal-300 font-semibold">Step 2</div>
              <h3 className="font-semibold mt-2">Verify & Train</h3>
              <p className="text-sm text-gray-300 mt-2">Upload documents; enroll for short courses and simulators.</p>
            </div>

            <div className="p-4 rounded-lg bg-black/6 border border-white/6">
              <div className="text-teal-300 font-semibold">Step 3</div>
              <h3 className="font-semibold mt-2">Deploy</h3>
              <p className="text-sm text-gray-300 mt-2">Fast placement with employer match and on-site onboarding.</p>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto bg-white/5 border border-white/6 rounded-2xl p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1">Short courses to boost deployability and wages.</p>

            <div className="mt-4 space-y-3">
              {memoTrainings.map((t) => (
                <div key={t.id} className="p-4 rounded-lg bg-black/6 border border-white/6 flex items-center justify-between">
                  <div>
                    <div className="font-semibold">{t.title}</div>
                    <div className="text-xs text-gray-300">{t.duration} • {t.type}</div>
                  </div>
                  <button className="rounded-md px-3 py-2 bg-linear-to-r from-teal-400 to-violet-500 text-black text-sm">Book</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-300 mt-1">Clear progression lanes across sectors.</p>
        ...
        <PromotionalBanner imageUrl={PROMO_IMG} title="Get Job-Ready — Fast Track Programs" subtitle="Enrol in short practical courses & simulator sessions. Certification increases your match rate." ctaText="Explore Programs" ctaHref="/trainings" />
        {/* ---------------------------------------------------------------- */}
        ...
        <section className="max-w-7xl mx-auto rounded-2xl p-6 bg-linear-to-r from-slate-900 to-black border border-white/6">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-gray-400">Brochures, checklists & FAQs</p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-lg bg-black/10 border border-white/6 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-300 mt-1">Placement overview & training</div>
              </div>
              <a href={HERO_POSTER} download className="px-3 py-2 rounded-md bg-linear-to-r from-teal-400 to-violet-500 text-black text-sm">Download</a>
            </div>

            <div className="p-4 rounded-lg bg-black/10 border border-white/6">
              <div className="font-semibold">Site Induction Checklist</div>
              <div className="text-xs text-gray-300 mt-1">PPE, documents & medical</div>
            </div>

            <div className="p-4 rounded-lg bg-black/10 border border-white/6">
              <div className="font-semibold">Document Help</div>
              <div className="text-xs text-gray-300 mt-1">Assistance for verification</div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white/4 rounded-2xl border border-white/6 p-6">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-gray-400">Upcoming training & hiring drives</p>
          </div>

          <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { title: "Simulator Bootcamp", date: "Dec 5, 2025", place: "Jharsuguda" },
              { title: "Mass Hiring Drive", date: "Dec 12, 2025", place: "Jamshedpur" },
              { title: "Safety Refresher", date: "Jan 8, 2026", place: "Bengaluru" },
            ].map((e, i) => (
              <div key={i} className="p-4 rounded-lg bg-black/6 border border-white/6 flex flex-col justify-between">
                <div>
                  <div className="font-semibold">{e.title}</div>
                  <div className="text-xs text-gray-300 mt-1">{e.date} • {e.place}</div>
                </div>
                <div className="mt-4">
                  <button className="rounded-md px-3 py-2 bg-linear-to-r from-teal-400 to-violet-500 text-black text-sm">Register</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="rounded-2xl p-6 bg-linear-to-r from-slate-900 to-black border border-white/6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-sm text-gray-300 mt-1">Help with registration, verification or document uploads</p>

              <div className="mt-4 space-y-3">
                <div className="p-4 rounded-lg bg-black/10 border border-white/6">
                  <div className="font-semibold">Registration Assistance</div>
                  <div className="text-sm text-gray-300 mt-1">Call our helpline for step-by-step help.</div>
                  <div className="mt-3 text-teal-300 font-semibold">+91 12345 67890</div>
                </div>

                <div className="p-4 rounded-lg bg-black/10 border border-white/6">
                  <div className="font-semibold">Document Verification</div>
                  <div className="text-sm text-gray-300 mt-1">Fast-track verification for certified candidates.</div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-2">
                <details className="bg-black/6 p-3 rounded-md border border-white/6">
                  <summary className="cursor-pointer font-medium">What documents do I need?</summary>
                  <div className="mt-2 text-sm text-gray-300">Government ID (Aadhaar/Voter), experience letters, medical if required.</div>
                </details>

                <details className="bg-black/6 p-3 rounded-md border border-white/6">
                  <summary className="cursor-pointer font-medium">Do you provide training?</summary>
                  <div className="mt-2 text-sm text-gray-300">Yes — short, role-specific courses and simulator sessions.</div>
                </details>
              </div>
            </div>
          </div>
        ...
        <footer id="contact" className="text-sm text-gray-400">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <div className="mt-2">Trusted workforce partner for heavy industries</div>
              <div className="mt-2">Helpline: <a href="tel:+911234567890" className="text-teal-300">+91 12345 67890</a></div>
            </div>
            ...
            <div className="flex gap-6">
              <div>
                ...
                <ul className="mt-2 space-y-1 text-gray-400">
                  ...
                  <li><Link to="/trainings" className="hover:underline">Training</Link></li>
                </ul>
              </div>

              <div>
                <div className="font-semibold text-white">Office</div>
                <div className="mt-2 text-gray-400">123 Industrial Park, Sample City</div>
              </div>
            </div>
          </div>

          <div className="mt-8 border-t border-white/6 pt-4 text-center text-xs text-gray-500">© {new Date().getFullYear()} Blue Global Force — All rights reserved.</div>
        </footer>
      </div>
    </main>
  );
  ...
  return (
    <section className="max-w-7xl mx-auto">
      ...
      <div className="relative">
        <div className="overflow-x-auto no-scrollbar snap-x snap-mandatory -mx-2 px-2 grid auto-cols-[minmax(280px,1fr)] grid-flow-col gap-4">
          {items.map((s) => (
            <article key={s.id} className="snap-start rounded-2xl overflow-hidden border border-white/6 bg-white/3 min-w-[320px] max-w-[380px]">
              <div className="h-44 w-full relative">
                <img src={s.image} alt={s.title} className="object-cover w-full h-full" loading="lazy" />
              ...
              <div className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-gray-300">{s.avgExperience}</div>
                </div>

                <p className="mt-3 text-sm text-gray-300"><strong className="text-white">Common:</strong> {s.commonRoles.join(", ")}</p>

                <div className="mt-4 flex gap-2">
                  <Link to={`/sectors/${s.id}`} className="text-sm text-teal-300 font-semibold">See roles →</Link>
                </div>
              </div>
            </article>
          ))}
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-6">
          ...
          <div>
            <Link to={ctaHref || "#"} className="inline-flex items-center gap-2 rounded-full px-5 py-3 bg-linear-to-r from-teal-400 to-violet-500 text-black font-semibold">
              {ctaText}
            </Link>
          </div>
        </div>
...
function ImpactSection() {
  const targets = useMemo(() => [
    { id: "t1", label: "Workers Deployed", value: 1520, accent: "text-teal-300" },
    { id: "t2", label: "Active Openings", value: 340, accent: "text-indigo-300" },
    { id: "t3", label: "Partner Companies", value: 28, accent: "text-amber-300" },
    { id: "t4", label: "Training Hours Delivered", value: 12400, accent: "text-pink-300" },
  ], []);
  ...
  return (
    <section className="max-w-7xl mx-auto rounded-2xl p-6 bg-white/5 border border-white/6">
      ...
      <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
        {targets.map((t, i) => (
          <div key={t.id} className="p-4 rounded-lg bg-black/6 border border-white/6 flex flex-col items-start">
            <div className={`text-3xl font-extrabold ${t.accent}`}>{counts[i].toLocaleString()}</div>
    ...
    <section className="max-w-7xl mx-auto rounded-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 gap-6 bg-transparent">
      ...
      <div className="relative hidden lg:block">
        ...
        <div className="absolute inset-0">
          <img src={OVERSEAS_COLLAGE} alt="Blue Global Force collage" className="object-cover w-full h-full" loading="lazy" />
      ...
      <div className="bg-white/4 border border-white/6 rounded-2xl p-8 flex flex-col justify-center">
        <div className="max-w-xl">
          <div className="text-sm uppercase text-teal-300 font-semibold">Welcome to Blue Global Force</div>
          <h2 className="text-3xl lg:text-4xl font-extrabold mt-3">Global Career Opportunities with Blue Global Force</h2>

          <p className="mt-6 text-gray-300 leading-relaxed">
            We connect Indian skilled workers with vetted international employers. Our expertise covers selection, compliance, pre-departure training, and on-site support — ensuring safe, legal and well-prepared deployments.
          </p>

          <p className="mt-4 text-sm text-gray-400">
            Whether you're seeking short-term projects or long-term placements, we help you prepare with role-specific training, certifications and verified employer matches.
          </p>
          ...
          <div className="mt-8">
            ...
            <div className="mt-2 w-full bg-white/8 h-3 rounded-full overflow-hidden">
              <div className="h-3 rounded-full" style={{ width: "98%", background: "linear-gradient(90deg,#0ee6d7,#4f46e5)" }} />
            </div>

            <div className="mt-4 flex gap-3">
              <Link to="/overseas" className="px-4 py-2 rounded-full bg-linear-to-r from-teal-400 to-violet-500 text-black font-semibold">Learn more</Link>
              <Link to="/contact" className="px-4 py-2 rounded-md border border-white/10">Contact us</Link>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="max-w-7xl mx-auto rounded-2xl overflow-hidden relative">
        ...
        <div className="bg-transparent border border-white/6 rounded-2xl p-8">
          <div className="text-center">
            <div className="text-sm uppercase text-gray-400">We deal in country</div>
